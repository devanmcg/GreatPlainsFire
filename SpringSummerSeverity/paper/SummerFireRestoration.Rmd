...
Grassland fuelbeds were less green during each of the spring fire seasons (Sentinel-2 NDVI 0.3-0.4) than in the summer (NDVI > 0.5), with the two seasons in which summer burns could not be completed being the most green (NDVI > 0.6; `r f_ref("sentinel")`A). 
Burn severity ranged widely within and across seasons and years (`r f_ref("sentinel")`B). 
Among the 32 spring burns, there was a general decline in burn severity with increasing fuelbed greenness. 
The low number of successful summer burns precluded a linear assessment, but despite having categorically higher greenness, summer burning effected the same range of variability in burn severity as in the spring (`r f_ref("sentinel")`B). 

Fuelbed greenness during successful burn seasons was close to the typical range over the 42-year historical period (`r f_ref("GreenRain")`A), while cumulative rainfall was at or below the typical historical range (`r f_ref("GreenRain")`B). 
Conversely, for years in which summer burns were not completed, both summer fuelbed greenness and accumulated rainfall by 1 August were atypically high relative to their historical ranges (`r f_ref("GreenRain")`).
...
Observed fire weather varied widely within and across the four study years, and generally spanned the entire range of historical values, although wind speeds on the day of prescribed burns never exceeded the historical median in either season, and only one burn day exceeded the historical median relative humidity (`r f_ref("weather")`). 
...
Few substantial changes in the fire environment appear to have occurred since the 1990s in either spring or summer burn seasons (`r f_ref("historical")`). 
...
<!-- But in the context of summer prescribed fire, when fuels are more green and windspeed generally lower (`r f_ref("historical")`), stubborn relative humidity is more of a barrier in the absence of mitigating factors.  -->
...
hist_cap <- f_nums(name = "historical", 
                   caption = "\\label{historical}Temporal trends in the phenology of the fire environment. Boxplots represent data from the first decade of the dataset used here (1990-1999) to the final decade (2012-2022), by burn season. Horizontal lines represent average conditions for the four study years (2017-2020).")
