...
```{r fig.cap="Study season data (points) plotted within the context of 42-year historical trends (boxplots). X indicates seasons prescribed fires were not completed. A: NDVI data retrieved from Landsat imagery. B: Annual growing season rainfall accummulated by two specific dates associated with the spring and summer burn seasons, respectively.", fig.width = 5, fig.height = 7}
p3 <-
LandsatSums %>%
...
```{r, fig.cap="Four fire weather variables plotted by burn day values (open triangles), study season means (open circles and X for seasons without completed burns), and 42-year historical trends (boxplots).", fig.height = 9}
RainyDays <- 
   WxData$Rainfall %>%
...
HistoricalData <- 
  ...
  ggplot() + theme_bw(14) + 
  geom_boxplot(data = HistoricalData  %>% filter(index == 'NDVI'),
               aes(x=season, 
                   y = Value),
               outliers = F, 
               staplewidth = 0.25) +
