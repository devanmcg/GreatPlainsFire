...
pacman::p_load(tidyverse, sf, lubridate, grid)
...
# GIS layers
  load(paste0(wd, './gis/Robjects/us_sf.Rdata'))
  load(paste0(wd, '/gis/Robjects/gp_l3.Rdata'))
  load(paste0(wd, '/gis/Robjects/GP_WF.Rdata'))
  load("C:/Users/devan.mcgranahan/GithubProjects/FireWeather/mapping/data/region_states.Rdata")

# weather data 
  load(paste0(wd, '/data/HistoricalWeather/NWGP_top_20.Rdata'))
  load(paste0(wd, '/data/PrelimAnalysis/FireWeekMeans.Rdata'))
  load(paste0(wd, '/data/PrelimAnalysis/FireEventWx.Rdata'))
```
...
inset_gg <-
  ggplot() + theme_map(14) +
    ...
    geom_sf(data = gp_l3 %>%
                    filter(US_L3NAME == "Northwestern Great Plains"), 
            fill = "lightblue") 
...
main_gg <-
  ggplot() + theme_map(14) +
    geom_sf(data = gp_l3 %>%
                    filter(US_L3NAME == "Northwestern Great Plains"), 
            fill = "lightblue") + 
    geom_sf(data = region_states, fill = NA) +
    geom_sf(data = fires, 
            pch=21, 
            size = 4, stroke = 1.25,
            fill = "darkblue", 
...
grid.newpage() 
main <- viewport(width = 1, height = 1, x = 0.5, y = 0.5) 
inset <- viewport(width = 0.5, height = 0.5, x = 0.2, y = 0.25)
print(main_gg, vp = main)
print(inset_gg, vp = inset)
# dev.off() 
```
